...
function submitCallback({ stages }, User) {
  ...
  return User.Tasks.createOKTargeting(
    artist, track, description, date, coverage
  );
}

class OKTargetingTask extends React.Component {
  componentDidMount() {
    this.props.onSave({
  ...
  render() {
    return (
      <div className="task-properties w-100">
        <table>
          <tbody>
            <tr className='task-title'><td colSpan='2'>Таргетинг в Odnoklassniki.ru</td></tr>
            <CommonTaskSettings
...
export default OKTargetingTask;
